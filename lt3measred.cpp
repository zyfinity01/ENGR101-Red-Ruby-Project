...
return(((totred > (2*totblue)) && (totred > (2*totgreen))) && ((notrubytotgreen > (totblue*1.5)) && (notrubytotblue > (totgreen*1.5)))); //the second half here, compares against the blue/green values in the diamond with the spot on the white piece of paper.
}
int main()
{
  int err = init(0);
  ...
  for(int countrun =0; countrun <1000; countrun=countrun+1)
  {
	...
	if(red_true(totred, totgreen, totblue)){
		...
		cout<<"OI THE RUBY IS STOLEN"<<endl;
	}
